...
private:
  ...
  // Helper for copying nodes into another BST
  ...
  // Helper to store the inorder traversal result in a vector
  void storeInorder(Node *node, vector<value_type> &nodes);
  // Helper to build a balanced tree from a vector of nodes
  Node *buildBalancedTree(vector<value_type> &nodes, int start, int end);
  // Helper for getAll()
  void getAllHelper(Node *node, const key_type &prefix,
                    vector<value_type> &result) const;
  // Helper for == and != operators
  bool isEqual(const Node *node1, const Node *node2) const;
  // Helper for erase()
  bool eraseHelper(Node *&node, const key_type &k);
